...
from sim2d.collisions import compute_collision
from sim2d.shapes import Floor
from sim2d.shapes import Shape
from sim2d.logger import EngineLogger, LoggingConfig
from sim2d.joints import Joint
from sim2d.joints import compute_joint_constraints
from sim2d.joints import JOINT_NUM_CONSTR
from sim2d.joints import JOINT_INT_TO_STR
from sim2d.joints import joint_to_int
from sim2d.gnn.dataset import NODE_FEATURE_DIMS
from sim2d.gnn.dataset import EDGE_FEATURE_DIMS
...
class Simulator(ABC):
    ...
    def _ensure_empty_edges(self, data):
        for edge_type_key, dim in EDGE_FEATURE_DIMS.items():
            if "joint_anchor" not in edge_type_key and edge_type_key not in data.edge_types:
                data[edge_type_key].edge_index = torch.zeros(
                    (2, 0), dtype=torch.long, device=self.device
                )
                data[edge_type_key].edge_attr = torch.zeros(
                    (0, dim), dtype=torch.float32, device=self.device
                )

    def _ensure_empty_edges_interim(self, data):
        for edge_type_key, dim in EDGE_FEATURE_DIMS.items():
            if (
                "joint_anchor" in edge_type_key or edge_type_key[1] == "contact"
            ) and edge_type_key not in data.edge_types:
                data[edge_type_key].edge_index = torch.zeros(
                    (2, 0), dtype=torch.long, device=self.device
                )
                data[edge_type_key].edge_attr = torch.zeros(
    ...
    def create_gnn_data_interim(self, state: torch.Tensor, constraints: torch.Tensor):
        gnn_data = HeteroData()
        gnn_data["object"].x = torch.zeros(
            (self.num_shapes, NODE_FEATURE_DIMS["object"]), dtype=torch.float32, device=self.device
        )
        ...
        # Contacts (identical to create_gnn_data)
        mask_contacts = ~mask_eq
        if mask_contacts.any():
            c_body = constraints["body_idx"][mask_contacts]
            c_neigh = constraints["neighbor_idx"][mask_contacts]
        ...
        # Joints: one joint_anchor node per joint
        # object/floor -> joint_anchor (Jacobian + error features)
        # joint_anchor -> object       (no features, carries lambda predictions)
        anchor_idx = 0
        to_anchor_obj = defaultdict(list)  # j_name -> [(src, dst, attr), ...]
        to_anchor_floor = defaultdict(list)  # j_name -> [(src, dst, attr), ...]
        from_anchor_obj = defaultdict(list)  # j_name -> [(src, dst), ...]
        ...
        if mask_eq.any():
            ...
            for joint in self.joints:
                ...
                # child -> anchor
                to_anchor_obj[j_name].append((joint.child_idx, anchor_idx, attr_child))
                # anchor -> child
                from_anchor_obj[j_name].append((anchor_idx, joint.child_idx))
                if joint.parent_idx == -1:
                    # floor -> anchor (floor's Jacobian rows)
                    to_anchor_floor[j_name].append((0, anchor_idx, attr_parent))
                else:
                    # parent -> anchor
                    to_anchor_obj[j_name].append((joint.parent_idx, anchor_idx, attr_parent))
                    # anchor -> parent
                    from_anchor_obj[j_name].append((anchor_idx, joint.parent_idx))
                curr += n_c
                anchor_idx += 1

        gnn_data["joint_anchor"].x = torch.zeros(
            (anchor_idx, 0), dtype=torch.float32, device=self.device
        )

        for j_name in JOINT_INT_TO_STR.values():
            if to_anchor_obj[j_name]:
                srcs, dsts, attrs = zip(*to_anchor_obj[j_name])
                gnn_data["object", j_name, "joint_anchor"].edge_index = torch.tensor(
                    [srcs, dsts], dtype=torch.long, device=self.device
                )
                gnn_data["object", j_name, "joint_anchor"].edge_attr = torch.stack(attrs)
            if to_anchor_floor[j_name]:
                srcs, dsts, attrs = zip(*to_anchor_floor[j_name])
                gnn_data["floor", j_name, "joint_anchor"].edge_index = torch.tensor(
                    [srcs, dsts], dtype=torch.long, device=self.device
                )
                gnn_data["floor", j_name, "joint_anchor"].edge_attr = torch.stack(attrs)
            if from_anchor_obj[j_name]:
                srcs, dsts = zip(*from_anchor_obj[j_name])
                gnn_data["joint_anchor", j_name, "object"].edge_index = torch.tensor(
                    [srcs, dsts], dtype=torch.long, device=self.device
                )
                gnn_data["joint_anchor", j_name, "object"].edge_attr = torch.zeros(
                    (len(srcs), 0), dtype=torch.float32, device=self.device
                )

        self._ensure_empty_edges_interim(gnn_data)
        return gnn_data

    def state_from_gnn_interim(self, gnn_output: tuple, constraints: dict) -> torch.Tensor:
        """Reads GNN output produced by a model trained on the interim joint architecture."""
        object_states, joints_decoded, lambdas_dict = gnn_output
        ...
        # Joint lambdas live on joint_anchor nodes. Joints are built in order, so
        # anchor i corresponds to self.joints[i]. Each anchor's prediction is padded
        # to max_constraints; we only read the first num_rows values.
        anchor_preds = joints_decoded if joints_decoded.numel() > 0 else None

        j_body_all = constraints["body_idx"][mask_eq]
        j_local_all = constraints["local_idx"][mask_eq]

        joint_constraint_offset = 0
        for anchor_i, joint in enumerate(self.joints):
            ...
            if anchor_preds is not None and anchor_i < anchor_preds.shape[0]:
                state_guess[body_idx, 3 + local_idxs_for_joint] = anchor_preds[anchor_i, :num_rows]

            joint_constraint_offset += num_rows
        return state_guess

    def state_from_gnn(self, gnn_output: tuple, constraints: dict) -> torch.Tensor:
        object_states, _, lambdas_dict = gnn_output
        state_guess = torch.zeros(self.solver.state_shape(constraints), device=self.device)
        state_guess[:, :3] = object_states
